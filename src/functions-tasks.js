...
function memoize(func) {
  const cash = {};
  return function cashing(...args) {
    const argKey = JSON.stringify(args);
    if (!cash[argKey]) {
      cash[argKey] = func(...args);
    }
    return cash[argKey];
  };
}
...
function logger(func, logFunc) {
  return function log(...args) {
    const argString = [];
    args.forEach((arg) => {
      argString.push(JSON.stringify(arg));
    });
    logFunc(`${func.name}(${argString.join(',')}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${argString.join(',')}) ends`);
    return result;
  };
}
